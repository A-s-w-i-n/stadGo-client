...
const OnwerstadiumList = () => {
  ...
  <div className="flex flex-row  " >
              {item.image.slice(0, 3).map((imageSrc, i) => (
               ...
               src={imageSrc}
               alt=""
               onClick={() => handleMainImageClick(item.maxcapacity, imageSrc)}
             />
              ))}
            </div>
            <img
  ...
  <div className="p-6 ml-16">
    ...
    <p className="font-normal text-gray-800 dark:text-gray-700 ml-6">{item.maxcapacity}</p>
  </div>
  <div className="flex mb-3 mt-7">
    <p className="font-extrabold w-40 text-xl ">Availabiltiy:</p>
    <p className="font-normal text-gray-800 dark:text-gray-700 ml-6">{item.fromdate} -- <span>{item.todate}</span></p>
  </div>
    
  <div className="flex mb-3 mt-7">
    <p className="font-extrabold w-40 text-xl">Locatoin:</p>
    <p className="font-normal text-gray-800 dark:text-gray-700 ml-6">{item.location}</p>
  </div>
  <div className="flex mb-3 mt-7">
    <p className="font-extrabold w-40 text-xl">Price:</p>
    <p className="font-normal text-gray-800 dark:text-gray-700 ml-6">{item.price}</p>
  </div>
  <div className="flex mb-3 mt-7">
    <p className="font-extrabold w-40 text-xl">Discription:</p>
    <p className="font-normal text-gray-800 dark:text-gray-700 ml-6">{item.discription}</p>
  </div>
